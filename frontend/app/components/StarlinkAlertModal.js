      ...
      <div className="w-[min(720px,94vw)] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <div className="border-b border-slate-200 bg-slate-50 px-5 py-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-extrabold uppercase text-cyan-700">Alertas Starlink</p>
              <h3 className="text-xl font-extrabold text-slate-950">Cortes próximos</h3>
        ...
        <div className="max-h-[54vh] overflow-y-auto p-5">
          <div className="space-y-3">
            {proximas.map((alerta) => (
              <div key={alerta.cuentaId} className={`rounded-xl border p-4 ${Number(alerta.diasRestantes) === 0 ? 'border-amber-200 bg-amber-50' : 'border-cyan-200 bg-cyan-50'}`}>
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <span className="rounded-full bg-white px-3 py-1 text-xs font-extrabold text-slate-700">{dayLabel(alerta.diasRestantes)}</span>
                </div>
